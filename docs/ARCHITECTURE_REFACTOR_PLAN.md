...
| **2** | **Stop state leaks** | ✅ DONE — fixed at the ROOT instead of the symptom. The leak existed only because in-sheet switches (WorkbookSidebar rows + the two new-workbook handlers) used same-segment `router.push`, which doesn't remount the page. Converted those 4 sites to **full navigation** (`window.location.assign`) → fresh document load = clean slate for every store + every mount hook re-runs. Chosen over the in-place `resetWorkbookScope()` registry because a correct in-place reset must coordinate sheetStore + workbookStore tabs + `hydrationVersion` + the pristine-guard — i.e. the exact fragile grid/tab/hydration machinery behind the flicker bugs (high-risk). Verified by a new `12-workbook-switch` e2e (full-nav probe + no-leak canary) + the data-entry/persistence suite (5/5 green). Instant-switch via the registry is deferred to post-grid-decomposition (Stage 7/8). | 1 | S | low |
| **3** | **Kill the re-render storm** | ✅ DONE (hot path) — converted the two ALWAYS-MOUNTED, keystroke-active whole-store subscribers, `SpreadsheetGrid` and `FormulaBar`, to `useShallow` selectors (same field sets → behavior-identical, re-render strictly less often). Verified the agent's list against live `develop`: 2 of its "9" were the already-deleted dead files; the rest are dialogs mounted only when open (user isn't typing) or use `workbookStore` (doesn't change on keystroke), so converting them is low-value and deferred. 547 unit + 5/5 e2e green. | — | S | low |
...
| **6** | **Schema truth** | ✅ DONE — generated a canonical `schema.live.generated.sql` from the LIVE DB (via the PostgREST OpenAPI spec, since the pooler is down), deprecated the 3 stale artifacts with headers, added `npm run db:schema:generate` + `npm run db:check-drift` (the drift guard) + `docs/setup/SCHEMA.md`. **The drift-check immediately found a real production bug: `workbooks.data` doesn't exist live, so cloud workbooks never persisted cell data to Supabase (silent localStorage fallback).** Fixed by migration `0013_workbooks_data_column.sql` (awaiting dashboard paste). | — | M | low (DB/process) |
